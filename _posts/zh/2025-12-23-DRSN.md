...
<pre id="bibtex-content-zhao2020" style="margin: 0; font-family: SFMono-Regular, Consolas, 'Liberation Mono', Menlo, monospace; font-size: 13px; line-height: 1.45; color: #24292e; white-space: pre;">@article{Zhao2020,
...
function copyBibtexStable(contentId, btnId) {
    ...
    function handleSuccess() {
        ...
        btn.style.color = '#22863a';      // 成功绿
        btn.style.borderColor = '#22863a';

        setTimeout(function() {
            btn.innerText = 'Copy'; // 复原为简短的 Copy
    ...
    function handleError(err) {
        ...
        alert('Press Ctrl+C to copy'); // 最后的兜底
    }

    // 1. 优先尝试现代 API (Navigator Clipboard)
    // 必须检查 navigator.clipboard 是否存在
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(content).then(handleSuccess).catch(function() {
            // 如果现代 API 失败（极少数情况），尝试降级方案
