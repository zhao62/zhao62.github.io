...
<style>
  ...
  .meta-btn {
    ...
    margin: 0;
    opacity: 0.95;
    white-space: nowrap; /* 防止按钮内文字换行，保持整洁 */
  }
  ...
  @media (max-width: 600px) {
    /* 容器调整：减小间距，利用 flex-wrap 让按钮自动折行 */
    .meta-container > div {
      gap: 6px !important; /* 缩小按钮之间的间隙 */
    }

    .meta-btn {
      /* 关键点：由 auto 改为 flex 布局控制 */
      /* flex: 1 0 auto; 意思是：
         1 -> 允许按钮伸缩填满空隙
         0 -> 空间不足时不压缩按钮内容
         auto -> 宽度根据内容自动调整
      */
      ...
      padding: 6px 8px;       /* 减小内边距，更紧凑 */
      max-width: 100%;        /* 防止极端情况溢出 */
    }
    
    /* 可选：如果您觉得某些按钮太长，可以在手机上把图标和文字的间距再调小 */
